...
func TestAPIDiagnosticsOAuthSharesSequenceAndCachesToken(t *testing.T) {
	...
	require.NotContains(t, output.String(), "auth token cache lookup")
	require.NotContains(t, output.String(), "auth token cache hit")
	require.Equal(t, 1, strings.Count(output.String(), "auth token fetch; target camunda_api"))
	require.Equal(t, 1, strings.Count(output.String(), "auth token cache store; target camunda_api"))
