...
func (s *Service) CreateProcessInstance(ctx context.Context, data d.ProcessInstanceData, opts ...services.CallOption) (d.ProcessInstanceCreation, error) {
	...
	if !cCfg.NoWait {
		...
		s.log.Info(fmt.Sprintf("process instance %s created for process definition %s (%s v%d, tenant %s); start registered %s, confirmed %s", pi.Key, pi.ProcessDefinitionKey, pi.BpmnProcessId, pi.ProcessDefinitionVersion, pi.TenantId, toolx.FormatNumericZoneTimestamp(pi.StartDate), toolx.FormatNumericZoneTimestamp(pi.StartConfirmedAt)))
	} else {
