...
func (s *Service) CreateProcessInstance(ctx context.Context, data d.ProcessInstanceData, opts ...services.CallOption) (d.ProcessInstanceCreation, error) {
	...
	if !cCfg.NoWait {
		...
		s.log.Info(fmt.Sprintf("pi %s created; pd %s %s v%d %s; start %s, confirmed %s", pi.Key, pi.ProcessDefinitionKey, pi.BpmnProcessId, pi.ProcessDefinitionVersion, pi.TenantId, toolx.FormatNumericZoneTimestamp(pi.StartDate), toolx.FormatNumericZoneTimestamp(pi.StartConfirmedAt)))
	} else {
		pi.StartDate = time.Now().UTC().Format(time.RFC3339)
		s.log.Info(fmt.Sprintf("pi %s create requested; pd %s %s v%d %s; start %s, no-wait", pi.Key, pi.ProcessDefinitionKey, pi.BpmnProcessId, pi.ProcessDefinitionVersion, pi.TenantId, toolx.FormatNumericZoneTimestamp(pi.StartDate)))
	}
	return pi, nil
}
...
func (s *Service) SearchForProcessInstancesPage(ctx context.Context, filter d.ProcessInstanceFilter, pageReq d.ProcessInstancePageRequest, opts ...services.CallOption) (d.ProcessInstancePage, error) {
	...
	processInstanceKeyFilter, err := common.NewProcessInstanceKeyEqFilterPtr(filter.Key)
	if err != nil {
	...
	processDefinitionVersionTagFilter, err := common.NewStringEqFilterPtr(filter.ProcessVersionTag)
	if err != nil {
		return d.ProcessInstancePage{}, fmt.Errorf("building process-definition-version-tag filter: %w", err)
	}
	startDateFilter, err := common.NewDateTimeRangeFilterPtr(startDateAfter, startDateBefore, nil)
	if err != nil {
		return d.ProcessInstancePage{}, fmt.Errorf("building start-date filter: %w", err)
	}
	endDateFilter, err := common.NewDateTimeRangeFilterPtr(endDateAfter, endDateBefore, endDateExists)
	if err != nil {
		return d.ProcessInstancePage{}, fmt.Errorf("building end-date filter: %w", err)
	}
	stateFilter, err := common.NewProcessInstanceStateEqFilterPtr(string(normalizeSearchState(filter.State)))
	if err != nil {
...
func pickProcessInstanceReportedTotal(page camundav88.SearchQueryPageResponse, itemCount int) *d.ProcessInstanceReportedTotal {
