...
func (s *Service) deleteExactProcessInstance(ctx context.Context, key string, cCfg *services.CallCfg, opts ...services.CallOption) (d.DeleteResponse, error) {
	resp, err := services.RetryCamundaMutation(ctx, s.log, "delete exact pi", func(ctx context.Context) (*camundav810.DeleteProcessInstanceResponse, *http.Response, []byte, error) {
		resp, err := s.cc.DeleteProcessInstanceWithResponse(ctx, key, camundav810.DeleteProcessInstanceJSONRequestBody{})
		if resp == nil {
			return resp, nil, nil, err
		}
