...
func (s *Service) Delete(ctx context.Context, resourceKey string, opts ...services.CallOption) (d.ResourceDeleteResponse, error) {
	...
	body := camundav810.DeleteResourceOpJSONRequestBody{DeleteHistory: boolPtr(true)}
	resp, err := services.RetryCamundaMutation(ctx, s.log, "delete pd", func(ctx context.Context) (*camundav810.DeleteResourceOpResponse, *http.Response, []byte, error) {
...
func (s *Service) waitForResourceHistoryDeletion(ctx context.Context, batchOperationKey string) (camundav810.BatchOperationStateEnum, error) {
	var state camundav810.BatchOperationStateEnum
...
func (s *Service) Deploy(ctx context.Context, units []d.DeploymentUnitData, opts ...services.CallOption) (d.Deployment, error) {
	...
	resp, err := services.RetryCamundaMutation(ctx, s.log, "deploy pd", func(ctx context.Context) (*camundav810.CreateDeploymentResponse, *http.Response, []byte, error) {
		_, _ = body.Seek(0, 0)
...
func (s *Service) waitForDeploymentConfirmation(ctx context.Context, dr camundav810.DeploymentResult, keys []string, vtenantID string, suppressDetailLogs bool, progress func(d.OpsProgressEvent)) error {
...
// processDefinitionDeployPoller adapts a v8.10 deployment response into the shared visibility poller.
// It keeps the version-specific response shape local while reusing the common confirmation behavior.
func (s *Service) processDefinitionDeployPoller(dr camundav810.DeploymentResult) func(context.Context) (poller.JobPollStatus, error) {
	return s.processDefinitionDeployPollerWithVisibilityCallback(deploymentProcessDefinitionKeys(dr), nil)
}

// deploymentProcessDefinitionKeys extracts process-definition keys from v8.10 deployment metadata.
func deploymentProcessDefinitionKeys(dr camundav810.DeploymentResult) []string {
	return resourcepayload.DeploymentProcessDefinitionKeys(dr.Deployments, func(dep camundav810.DeploymentMetadataResult) string {
