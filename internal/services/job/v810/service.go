...
// Service adapts Camunda 8.10 job endpoints to the version-neutral job API.
type Service struct {
	c   GenJobClient
	cfg *config.Config
	log *slog.Logger
...
// Client returns the generated Camunda job client used by this service.
...
// Option customizes v8.10 job service construction.
type Option func(*Service)

// WithClient overrides the generated Camunda job client, primarily for service tests.
...
// New prepares a v8.10 job service with the generated Camunda v2 client.
...
// GetJob finds a single visible job by key through the v8.10 search endpoint.
func (s *Service) GetJob(ctx context.Context, key string, opts ...services.CallOption) (d.Job, error) {
	...
	resp, err := s.c.SearchJobsWithResponse(ctx, camundav810.SearchJobsJSONRequestBody{
		Filter: &camundav810.JobFilter{
			JobKey: jobKeyFilter,
		},
		Page: &page,
	})
...
func trimJobSearchPageResults(items []camundav810.JobSearchResult, page camundav810.SearchQueryPageResponse, from int32, pageSize int32) []camundav810.JobSearchResult {
...
func pickJobSearchOverflowState(page camundav810.SearchQueryPageResponse, from int32, itemCount int, pageSize int32) d.ProcessInstanceOverflowState {
	if itemCount == 0 {
...
// UpdateJob submits retry or timeout changes and optionally confirms the requested retries.
func (s *Service) UpdateJob(ctx context.Context, request d.JobUpdateRequest, opts ...services.CallOption) (d.JobUpdateResult, error) {
	...
	body := camundav810.UpdateJobJSONRequestBody{
		Changeset: camundav810.JobChangeset{
			Retries: request.Retries,
			Timeout: request.TimeoutMillis,
		},
	}
	resp, err := services.RetryCamundaMutation(ctx, s.log, "update job", func(ctx context.Context) (*camundav810.UpdateJobResponse, *http.Response, []byte, error) {
		resp, err := s.c.UpdateJobWithResponse(ctx, camundav810.JobKey(request.Key), body)
		if resp == nil {
...
// SubmitJobWorkerOutcome submits the requested worker outcome through the native v8.10 lifecycle endpoint.
...
func (s *Service) submitJobTechnicalFailure(ctx context.Context, request d.JobWorkerOutcomeRequest) (d.JobWorkerOutcomeResult, error) {
	...
	resp, err := services.RetryCamundaMutation(ctx, s.log, "fail job", func(ctx context.Context) (*camundav810.FailJobResponse, *http.Response, []byte, error) {
		resp, err := s.c.FailJobWithResponse(ctx, camundav810.JobKey(request.Key), body)
		if resp == nil {
			return resp, nil, nil, err
		}
		return resp, resp.HTTPResponse, resp.Body, err
...
func (s *Service) submitJobBPMNError(ctx context.Context, request d.JobWorkerOutcomeRequest) (d.JobWorkerOutcomeResult, error) {
	...
	resp, err := services.RetryCamundaMutation(ctx, s.log, "throw job BPMN error", func(ctx context.Context) (*camundav810.ThrowJobErrorResponse, *http.Response, []byte, error) {
		resp, err := s.c.ThrowJobErrorWithResponse(ctx, camundav810.JobKey(request.Key), body)
		if resp == nil {
			return resp, nil, nil, err
		}
		return resp, resp.HTTPResponse, resp.Body, err
...
func (s *Service) submitJobCompletion(ctx context.Context, request d.JobWorkerOutcomeRequest) (d.JobWorkerOutcomeResult, error) {
	...
	resp, err := services.RetryCamundaMutation(ctx, s.log, "complete job", func(ctx context.Context) (*camundav810.CompleteJobResponse, *http.Response, []byte, error) {
		resp, err := s.c.CompleteJobWithResponse(ctx, camundav810.JobKey(request.Key), body)
		if resp == nil {
			return resp, nil, nil, err
		}
		return resp, resp.HTTPResponse, resp.Body, err
...
// newFailJobRequestBody keeps generated v8.10 failure request details inside the
// service adapter while preserving explicit zero retries.
func newFailJobRequestBody(request d.JobWorkerOutcomeRequest) camundav810.FailJobJSONRequestBody {
	body := camundav810.FailJobJSONRequestBody{
		Retries:      request.Retries,
...
func newThrowJobErrorRequestBody(request d.JobWorkerOutcomeRequest) camundav810.ThrowJobErrorJSONRequestBody {
	body := camundav810.ThrowJobErrorJSONRequestBody{
...
func newCompleteJobRequestBody(request d.JobWorkerOutcomeRequest) camundav810.CompleteJobJSONRequestBody {
	variables := request.Variables
	if variables == nil {
		// Camunda's generated completion request requires a variables object even when no variables are added.
		variables = map[string]any{}
	}
	return camundav810.CompleteJobJSONRequestBody{
		Variables: &variables,
	}
}
