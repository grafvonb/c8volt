...
func (s *Service) SearchProcessDefinitionsPage(ctx context.Context, filter d.ProcessDefinitionFilter, pageReq d.ProcessDefinitionPageRequest, opts ...services.CallOption) (d.ProcessDefinitionPage, error) {
	...
	items := toolx.MapSlice(payload.Items, fromProcessDefinitionResult)
	if cCfg.WithStat {
		for i := range items {
			if items[i].Key == "" {
				continue
	...
	return d.ProcessDefinitionPage{
		Items:         items,
		Request:       pageReq,
		OverflowState: pickProcessDefinitionOverflowState(payload.Page, pageReq, len(items)),
		ReportedTotal: newProcessDefinitionReportedTotal(payload.Page),
		EndCursor:     processDefinitionEndCursor(payload.Page),
	}, nil
}
...
func processDefinitionEndCursor(page camundav88.SearchQueryPageResponse) string {
	if page.EndCursor == nil {
		return ""
	}
	return string(*page.EndCursor)
...
// newProcessDefinitionReportedTotal converts v8.8 page metadata into domain total metadata.
func newProcessDefinitionReportedTotal(page camundav88.SearchQueryPageResponse) *d.ProcessDefinitionReportedTotal {
...
func pickProcessDefinitionOverflowState(page camundav88.SearchQueryPageResponse, req d.ProcessDefinitionPageRequest, itemCount int) d.ProcessInstanceOverflowState {
	if itemCount == 0 {
...
func searchProcessDefinitionsRequest(tenantID string, filter d.ProcessDefinitionFilter, pageReq d.ProcessDefinitionPageRequest) (camundav88.SearchProcessDefinitionsJSONRequestBody, error) {
