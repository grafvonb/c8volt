...
func (s *Service) SearchUserTaskEffectiveVariablesPage(ctx context.Context, key string, pageReq d.UserTaskVariablePageRequest, opts ...services.CallOption) (d.UserTaskVariablePage, error) {
	...
	sort := []camundav810.UserTaskVariableSearchQuerySortRequest{{
		Field: camundav810.UserTaskVariableSearchQuerySortRequestFieldName,
		Order: &order,
	}}
	body := camundav810.SearchUserTaskEffectiveVariablesJSONRequestBody{
		Page: &camundav810.OffsetPagination{From: &pageReq.From, Limit: &pageReq.Size},
		Sort: &sort,
	}
	resp, err := s.cc.SearchUserTaskEffectiveVariablesWithResponse(
		ctx,
		camundav810.UserTaskKey(key),
		&camundav810.SearchUserTaskEffectiveVariablesParams{TruncateValues: &truncateValues},
		body,
	)
	if err != nil {
