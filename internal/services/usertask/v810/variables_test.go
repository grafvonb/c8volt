...
// TestService_SearchUserTaskEffectiveVariablesPage_RequestsNativeOffsetPage verifies the route and complete native request contract.
func TestService_SearchUserTaskEffectiveVariablesPage_RequestsNativeOffsetPage(t *testing.T) {
	...
	var requestBody camundav810.SearchUserTaskEffectiveVariablesJSONRequestBody
	...
	svc, err := v810.New(&config.Config{APIs: config.APIs{Camunda: config.API{BaseURL: server.URL + "/v2"}}}, server.Client(), slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	require.Equal(t, camundav810.UserTaskVariableSearchQuerySortRequestFieldName, (*requestBody.Sort)[0].Field)
	require.NotNil(t, (*requestBody.Sort)[0].Order)
	require.Equal(t, camundav810.ASC, *(*requestBody.Sort)[0].Order)
...
func TestService_SearchUserTaskEffectiveVariablesPage_PreservesCappedTotal(t *testing.T) {
	svc := newTestService(t, &mockUserTaskClient{
		searchUserTaskEffectiveVariablesWithResponse: func(_ context.Context, key camundav810.UserTaskKey, params *camundav810.SearchUserTaskEffectiveVariablesParams, _ camundav810.SearchUserTaskEffectiveVariablesJSONRequestBody, _ ...camundav810.RequestEditorFn) (*camundav810.SearchUserTaskEffectiveVariablesResponse, error) {
			require.Equal(t, camundav810.UserTaskKey("task-a"), key)
...
func TestService_SearchUserTaskEffectiveVariablesPage_RejectsBackendAndMalformedResponses(t *testing.T) {
	...
	tests := []struct {
		name      string
		response  *camundav810.SearchUserTaskEffectiveVariablesResponse
		transport error
		want      error
	}{
...
func effectiveVariablesSuccess(body []byte) *camundav810.SearchUserTaskEffectiveVariablesResponse {
	response := effectiveVariablesResponse(http.StatusOK, body)
	response.JSON200 = &camundav810.VariableSearchQueryResult{}
	return response
}

// effectiveVariablesResponse creates stable HTTP metadata for effective-variable adapter tests.
func effectiveVariablesResponse(status int, body []byte) *camundav810.SearchUserTaskEffectiveVariablesResponse {
	return &camundav810.SearchUserTaskEffectiveVariablesResponse{
		Body:         body,
