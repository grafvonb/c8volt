...
// TestService_GetNativeUserTask_UsesDirectGetAndPreservesForeignTenant verifies V89 native reads never search or fall back.
func TestService_GetNativeUserTask_UsesDirectGetAndPreservesForeignTenant(t *testing.T) {
	...
	svc := newTestServiceWithTasklist(t, &mockUserTaskCamundaClient{
		getUserTaskWithResponse: func(_ context.Context, key camundav89.UserTaskKey, _ ...camundav89.RequestEditorFn) (*camundav89.GetUserTaskResponse, error) {
			require.Equal(t, camundav89.UserTaskKey(payload.UserTaskKey), key)
...
// TestService_GetNativeUserTask_RejectsBackendAndMalformedResponses covers V89 native failures without a successful partial task.
func TestService_GetNativeUserTask_RejectsBackendAndMalformedResponses(t *testing.T) {
	tests := []struct {
		name      string
		response  *camundav89.GetUserTaskResponse
		transport error
		wantError error
	}{
...
func nativeUserTaskResultPtr(key string, state camundav89.UserTaskStateEnum, processInstanceKey string) *camundav89.UserTaskResult {
	return &camundav89.UserTaskResult{UserTaskKey: key, State: state, ProcessInstanceKey: processInstanceKey}
}

// nativeUserTaskHTTPResponse creates a generated V89 response with the requested status and payload.
func nativeUserTaskHTTPResponse(status int, payload *camundav89.UserTaskResult) *camundav89.GetUserTaskResponse {
	return &camundav89.GetUserTaskResponse{Body: []byte(`{"message":"backend response"}`), HTTPResponse: newHTTPResponse(http.MethodGet, "https://camunda.local/v2/user-tasks/2251799815391233", status, http.StatusText(status)), JSON200: payload}
}
