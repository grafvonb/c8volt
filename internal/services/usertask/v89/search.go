...
// SearchUserTasksPage executes one backend-filtered v8.9 user-task search request.
...
// userTaskSearchRequest maps stable filters to the scalar selector shapes used by v8.9.
func userTaskSearchRequest(query d.UserTaskSearchQuery, pageReq d.UserTaskPageRequest, tenantID string) (camundav89.SearchUserTasksJSONRequestBody, error) {
	...
	filter := &camundav89.UserTaskFilter{
		...
		ProcessDefinitionId:  stringPtrAs[camundav89.ProcessDefinitionId](query.BpmnProcessId),
		ElementId:            stringPtrAs[camundav89.ElementId](query.ElementId),
		State:                state,
		Assignee:             assignee,
...
func newUserTaskStateEqFilterPtr(value string) (*camundav89.UserTaskStateFilterProperty, error) {
	if value == "" {
		return nil, nil
	}
	filter := new(camundav89.UserTaskStateFilterProperty)
	if err := filter.FromUserTaskStateFilterProperty0(camundav89.UserTaskStateEnum(value)); err != nil {
		return nil, err
	}
	return filter, nil
}
...
func newUserTaskSearchPageRequest(pageReq d.UserTaskPageRequest) camundav89.SearchQueryPageRequest {
	page := camundav89.SearchQueryPageRequest{}
	switch {
	case pageReq.After != "":
		_ = page.FromCursorForwardPagination(camundav89.CursorForwardPagination{After: camundav89.EndCursor(pageReq.After), Limit: &pageReq.Size})
	case pageReq.From != 0:
		_ = page.FromOffsetPagination(camundav89.OffsetPagination{From: &pageReq.From, Limit: &pageReq.Size})
	default:
		_ = page.FromLimitPagination(camundav89.LimitPagination{Limit: &pageReq.Size})
	}
	return page
}

// userTaskSearchPage maps backend items and normalizes total and continuation metadata.
func userTaskSearchPage(payload camundav89.UserTaskSearchQueryResult, pageReq d.UserTaskPageRequest) (d.UserTaskSearchPage, error) {
