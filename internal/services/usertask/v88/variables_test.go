...
// TestService_SearchUserTaskEffectiveVariablesPage_RequestsNativeOffsetPage verifies v8.8 delegates authorization to the selected-key endpoint and preserves returned scope metadata.
func TestService_SearchUserTaskEffectiveVariablesPage_RequestsNativeOffsetPage(t *testing.T) {
	...
	var requestBody camundav88.SearchUserTaskEffectiveVariablesJSONRequestBody
	...
	svc, err := v88.New(&config.Config{APIs: config.APIs{Camunda: config.API{BaseURL: server.URL + "/v2"}}}, server.Client(), slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	require.Equal(t, camundav88.UserTaskVariableSearchQuerySortRequestFieldName, (*requestBody.Sort)[0].Field)
	require.NotNil(t, (*requestBody.Sort)[0].Order)
	require.Equal(t, camundav88.ASC, *(*requestBody.Sort)[0].Order)
...
func TestService_SearchUserTaskEffectiveVariablesPage_PreservesCappedTotal(t *testing.T) {
	svc := newTestService(t, &mockUserTaskCamundaClient{
		searchUserTaskEffectiveVariablesWithResponse: func(_ context.Context, key camundav88.UserTaskKey, params *camundav88.SearchUserTaskEffectiveVariablesParams, _ camundav88.SearchUserTaskEffectiveVariablesJSONRequestBody, _ ...camundav88.RequestEditorFn) (*camundav88.SearchUserTaskEffectiveVariablesResponse, error) {
			require.Equal(t, camundav88.UserTaskKey("task-a"), key)
...
func TestService_SearchUserTaskEffectiveVariablesPage_RejectsBackendAndMalformedResponses(t *testing.T) {
	...
	tests := []struct {
		name      string
		response  *camundav88.SearchUserTaskEffectiveVariablesResponse
		transport error
		want      error
	}{
...
func effectiveVariablesSuccess(body []byte) *camundav88.SearchUserTaskEffectiveVariablesResponse {
	response := effectiveVariablesResponse(http.StatusOK, body)
	response.JSON200 = &camundav88.VariableSearchQueryResult{}
	return response
}

// effectiveVariablesResponse creates stable HTTP metadata for effective-variable adapter tests.
func effectiveVariablesResponse(status int, body []byte) *camundav88.SearchUserTaskEffectiveVariablesResponse {
	return &camundav88.SearchUserTaskEffectiveVariablesResponse{
		Body:         body,
