...
func (s *Service) SearchUserTaskEffectiveVariablesPage(ctx context.Context, key string, pageReq d.UserTaskVariablePageRequest, opts ...services.CallOption) (d.UserTaskVariablePage, error) {
	...
	sort := []camundav88.UserTaskVariableSearchQuerySortRequest{{
		Field: camundav88.UserTaskVariableSearchQuerySortRequestFieldName,
		Order: &order,
	}}
	body := camundav88.SearchUserTaskEffectiveVariablesJSONRequestBody{
		Page: &camundav88.OffsetPagination{From: &pageReq.From, Limit: &pageReq.Size},
		Sort: &sort,
	}
	resp, err := s.cc.SearchUserTaskEffectiveVariablesWithResponse(
		ctx,
		camundav88.UserTaskKey(key),
		&camundav88.SearchUserTaskEffectiveVariablesParams{TruncateValues: &truncateValues},
		body,
	)
	if err != nil {
