...
// Client returns the generated Camunda tenant client used by the v8.10 service.
func (s *Service) Client() GenTenantClient { return s.c }

// Config returns the normalized service configuration used by the v8.10 tenant service.
func (s *Service) Config() *config.Config { return s.cfg }

// Logger returns the service logger used by the v8.10 tenant service.
...
// New prepares a v8.10 tenant service with the generated Camunda v2 tenant client.
...
func (s *Service) GetTenant(ctx context.Context, tenantID string, opts ...services.CallOption) (d.Tenant, error) {
	...
	resp, err := s.c.GetTenantWithResponse(ctx, camundav810.TenantId(tenantID))
	if err != nil {
		return d.Tenant{}, err
	}
	payload, err := common.RequirePayload(resp.HTTPResponse, resp.Body, resp.JSON200)
...
func searchTenantsRequest(size int32) camundav810.SearchTenantsJSONRequestBody {
	page := camundav810.SearchQueryPageRequest{}
	_ = page.FromLimitPagination(camundav810.LimitPagination{Limit: &size})
	order := camundav810.ASC
	sort := []camundav810.TenantSearchQuerySortRequest{
		{Field: camundav810.TenantSearchQuerySortRequestFieldName, Order: &order},
		{Field: camundav810.TenantSearchQuerySortRequestFieldTenantId, Order: &order},
	}
	return camundav810.SearchTenantsJSONRequestBody{
		Page: &page,
		Sort: &sort,
	}
}
