...
import (
	...
	v810 "github.com/grafvonb/c8volt/internal/services/tenant/v810"
...
func (m *mockTenantClient) SearchTenantsWithResponse(ctx context.Context, body camundav810.SearchTenantsJSONRequestBody, reqEditors ...camundav810.RequestEditorFn) (*camundav810.SearchTenantsResponse, error) {
	args := m.Called(ctx, body)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*camundav810.SearchTenantsResponse), args.Error(1)
}

// GetTenantWithResponse records keyed tenant lookup calls and returns the configured mock response.
func (m *mockTenantClient) GetTenantWithResponse(ctx context.Context, tenantId camundav810.TenantId, reqEditors ...camundav810.RequestEditorFn) (*camundav810.GetTenantResponse, error) {
	args := m.Called(ctx, tenantId)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*camundav810.GetTenantResponse), args.Error(1)
}

// Verifies v8.10 tenant search builds the native request, converts results, filters locally, and maps failures.
func TestService_SearchTenants(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mockTenantClient) {
				resp := &camundav810.SearchTenantsResponse{
					HTTPResponse: newHTTPResponse(http.MethodPost, "https://example.com/tenants/search", http.StatusOK, "200 OK"),
					JSON200: &camundav810.TenantSearchQueryResult{
						Items: []camundav810.TenantResult{
							{TenantId: "tenant-b", Name: "Beta"},
							{TenantId: "tenant-a", Name: "Alpha", Description: &desc},
						},
					},
				}
				m.On("SearchTenantsWithResponse", mock.Anything, mock.MatchedBy(func(body camundav810.SearchTenantsJSONRequestBody) bool {
		...
		},
		{
			name: "client error",
			setupMock: func(m *mockTenantClient) {
				m.On("SearchTenantsWithResponse", mock.Anything, mock.Anything).Return((*camundav810.SearchTenantsResponse)(nil), mockErr)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &mockTenantClient{}
			tt.setupMock(m)
			svc, err := v810.New(testx.TestConfig(t), &http.Client{}, slog.New(slog.NewTextHandler(io.Discard, nil)), v810.WithClient(m))
...
// Verifies v8.10 keyed tenant lookup converts native results and maps native response failures.
func TestService_GetTenant(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "success",
			setupMock: func(m *mockTenantClient) {
				resp := &camundav810.GetTenantResponse{
					HTTPResponse: newHTTPResponse(http.MethodGet, "https://example.com/tenants/tenant-a", http.StatusOK, "200 OK"),
					JSON200: &camundav810.TenantResult{
						TenantId:    "tenant-a",
						Name:        "Alpha",
						Description: &desc,
					},
				}
				m.On("GetTenantWithResponse", mock.Anything, camundav810.TenantId("tenant-a")).Return(resp, nil)
			},
		...
		},
		{
			...
			setupMock: func(m *mockTenantClient) {
				...
				m.On("GetTenantWithResponse", mock.Anything, camundav810.TenantId("missing")).Return(resp, nil)
			},
		...
		},
		{
			name: "client error",
			setupMock: func(m *mockTenantClient) {
				m.On("GetTenantWithResponse", mock.Anything, camundav810.TenantId("tenant-a")).Return((*camundav810.GetTenantResponse)(nil), mockErr)
			},
			expectedError: mockErr,
		},
	}
