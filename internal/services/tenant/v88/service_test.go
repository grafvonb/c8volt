...
func (m *mockTenantClient) GetAuthenticationWithResponse(ctx context.Context, reqEditors ...camundav88.RequestEditorFn) (*camundav88.GetAuthenticationResponse, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*camundav88.GetAuthenticationResponse), args.Error(1)
}
...
func TestService_SearchTenants(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(m *mockTenantClient) {
				searchResp := &camundav88.SearchTenantsResponse{
					HTTPResponse: newHTTPResponse(http.MethodPost, "https://example.com/tenants/search", http.StatusOK, "200 OK"),
					JSON200:      &camundav88.TenantSearchQueryResult{},
				}
				authResp := &camundav88.GetAuthenticationResponse{
					HTTPResponse: newHTTPResponse(http.MethodGet, "https://example.com/authentication/me", http.StatusOK, "200 OK"),
					JSON200: &camundav88.CamundaUserResult{
						Tenants: []camundav88.TenantResult{
							{TenantId: "tenant-a", Name: "Alpha", Description: &desc},
						},
					},
				}
