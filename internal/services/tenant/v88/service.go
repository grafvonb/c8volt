...
func (s *Service) GetTenant(ctx context.Context, tenantID string, opts ...services.CallOption) (d.Tenant, error) {
	...
	resp, err := s.c.GetTenantWithResponse(ctx, camundav88.TenantId(tenantID))
	if err != nil {
		return d.Tenant{}, err
	}
	payload, err := common.RequirePayload(resp.HTTPResponse, resp.Body, resp.JSON200)
...
func searchTenantsRequest(size int32) camundav88.SearchTenantsJSONRequestBody {
	...
	sort := []camundav88.TenantSearchQuerySortRequest{
		{Field: camundav88.TenantSearchQuerySortRequestFieldName, Order: &order},
		{Field: camundav88.TenantSearchQuerySortRequestFieldTenantId, Order: &order},
	}
	return camundav88.SearchTenantsJSONRequestBody{
		Page: &page,
		Sort: &sort,
	}
}
