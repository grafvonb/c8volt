...
// TestDarwinCmdTerminalAllocator verifies that the allocated slave is a canonical, no-echo terminal.
func TestDarwinCmdTerminalAllocator(t *testing.T) {
	...
	settings, err := unix.IoctlGetTermios(int(allocation.slave.Fd()), unix.TIOCGETA)
	require.NoError(t, err)
...
// TestDarwinCmdTerminalAllocatorClosesMasterWhenSlaveOpenFails verifies partial allocation cleanup.
func TestDarwinCmdTerminalAllocatorClosesMasterWhenSlaveOpenFails(t *testing.T) {
	var masterFD int
	openCalls := 0
	allocator := darwinCmdTerminalAllocator{
