...
- Human tenant-context rendering lives in `cmd/cmd_views_tenant_context.go`; it suppresses JSON, keys-only, and quiet modes, routes prefix-free warning messages through the shared warning renderer, and skips the duplicate unfiltered warning because the semantic line already carries that wording.
...
- Tenant-context warning messages must not embed `WARN` or `WARNING:`. Use the shared warning renderer for normal human output and pass the prefix-free message to durable ops warning channels so logger-backed output cannot become `WARN WARNING:`.
