...
- **FR-007a**: Direct Camunda API setup MAY intentionally create inconsistent cluster state when c8volt commands are designed to prevent that state, such as deleting a call-activity parent directly to create orphan children. Such setup MUST be classified as test-only corruption unless the gap explicitly calls for a product-facing setup command.
...
- **Controlled Corruption Setup**: A test-only direct Camunda API action that deliberately bypasses c8volt safety behavior to create inconsistent state that c8volt must later detect, report, or repair.
