...
2. **Given** suite-owned incidents with related jobs, **When** repair, retry, fail, timeout, and BPMN error scenarios run, **Then** the suite verifies observable post-state or records an explicit unsupported or missing-fixture proposal.
...
Release validators can prove listener-related flags and BPMN error job behavior with real deployed process models rather than only checking flag wiring or keeping the need as a proposal.
...
**Independent Test**: Run the listener and BPMN error slice with embedded models when available, falling back to a recorded proposal only when c8volt lacks the command or embedded model needed to create the required state.
...
3. **Given** no suitable c8volt command or embedded model exists, **When** the slice reaches that setup need, **Then** it writes an actionable proposal for the missing command or embedded BPMN fixture instead of silently skipping coverage.
...
Maintainers can inspect a single real-state coverage matrix and proposal evidence to understand what is live-covered, what remains proposal-backed, and what must be revisited for later Camunda minor releases.
...
**Independent Test**: Run the proposal and matrix verification slice and confirm every known missing command setup or embedded BPMN fixture is represented in aggregate evidence with affected commands, versions, and operator value.

**Acceptance Scenarios**:

1. **Given** existing command-family proposal records, **When** aggregate proposal evidence is written, **Then** all current family gaps, including ops repair gaps, appear in the aggregate command and embedded BPMN proposal reports.
2. **Given** a new real-state gap is discovered, **When** the suite cannot create the state through c8volt commands or embedded models, **Then** the gap is added to proposal evidence with a concrete required state and affected command list.
...
- A required state cannot be produced by an existing c8volt command.
- A required process behavior cannot be produced by an existing embedded BPMN model.
...
- **FR-007**: Any direct Camunda API setup MUST be recorded as a command-extension proposal with required state, affected commands, affected versions, and operator value.
- **FR-008**: Process definitions used by real-state tests MUST prefer existing embedded c8volt models.
- **FR-009**: Any missing embedded process behavior, including listener, BPMN error, repair, retention, or partial-failure fixtures, MUST be recorded as an embedded BPMN proposal.
...
- **FR-012**: The suite MUST prove listener-related command behavior with real listener state or record the missing setup as proposal evidence.
- **FR-013**: The suite MUST prove deterministic retention, purge, delete, cancel, resolve, repair, and cleanup candidates with real post-state verification or explicit no-wait, accepted, retained, cleanup-failed, skipped, unsupported, or proposed outcomes.
...
- **FR-018**: Aggregate proposal reports MUST include all known command-family and embedded BPMN gaps from the baseline, volume, and real-state suites.
- **FR-019**: The suite MUST maintain a feature-local coverage matrix showing each priority topic, current evidence level, target real-state proof, and remaining proposal-backed work.
...
- **Gap Proposal**: A durable record for missing command setup capability or missing embedded BPMN behavior that blocks deeper live coverage.
...
- **SC-001**: Every topic listed in the feature coverage matrix has an explicit status of live-covered, partially live-covered, no-match only, proposal-backed, or not yet started.
...
- **SC-004**: At least one incident scenario proves active incidents with related job evidence or records the missing setup as proposal evidence.
- **SC-005**: Listener and BPMN error topics are either live-covered on Camunda 8.9 or represented by proposal records with affected commands and fixture needs.
...
- **SC-008**: Aggregate command and embedded BPMN proposal reports include ops repair gaps and all other known setup gaps.
