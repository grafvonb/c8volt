...
- Make integration targets now ask for confirmation before running because slices may mutate real Camunda state. The shared `integration-test-confirm` prerequisite prompts once per `make` invocation, including aggregate targets. Automation skips it with `IT_CONFIRM=0`.
