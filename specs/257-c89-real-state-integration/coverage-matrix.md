...
| Job retries, timeout, fail, no-wait | partially live-covered: retries confirmed, fail/no-wait submitted, timeout dry-run planned; activated timeout mutation is proposal-backed | Observable job post-state after update paths, or explicit no-wait/submitted evidence | Add c8volt setup support for activated jobs so `update job --timeout` can be confirmed |
| `update job --throw-bpmn-error` | proposal-backed with live dry-run plan and unchanged job-state verification on Camunda 8.9 | BPMN error-capable job drives an observable error path | Add embedded catchable BPMN error fixture and c8volt setup path for confirmed mutation |
