...
| Ops report parity | partially live-covered: retention target writes JSON report evidence for non-empty real candidates and proves dry-run plus confirmed stdout/report agreement | Real-state scenarios prove report/stdout parity on non-empty candidates | Extend report parity to purge and repair scenarios |
