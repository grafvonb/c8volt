...
Add Camunda 8.10 as one ordinary c8volt compatibility line (`toolx.V810`, canonical value `8.10`) and disclose that the initial generated source is Camunda `8.10.0-alpha4`. The original delivery retained V88 as default; issue #277 later promotes the active default to V89 without changing V810. Prepare one isolated unified client under `internal/clients/camunda/v810/camunda`, record deterministic provenance, add native `v810` adapters and factory cases for all eleven version-aware service families, replace exact-v8.9 checks with named capability predicates, explicitly map v8.10 to native C810 production fixtures, and preserve version-neutral facade, CLI, polling, retry, confirmation, output, and error contracts. Generated clients for v8.7-v8.9 and everything under `integration/` remain unchanged.
...
**Constraints**: Initial upstream tag `8.10.0-alpha4` at commit `4a76f06c9df8ad0a6c64fe88b92c618c2ebf2ef6`; one `V810`/`v810` family only; `CurrentCamundaVersion` is V89 after issue #277 superseded the original V88 default; no v8.10 Operate, Tasklist, or Administration SM clients; no changes to v8.7-v8.9 generated clients or `integration/`
...
└── fixture_compatibility.go         # explicit V810 -> C810_ mapping
...
1. Add `toolx.V810`, the four required aliases, and supported/implemented membership; reject prerelease-tag aliases. The original V88 default is superseded by the active V89 default from issue #277.
...
5. Add an explicit production-fixture mapping from V810 to `C810_`; retain reporting as `8.10`, keep live integration selection unchanged, and do not add an 8.10 integration profile.
...
- **Fixture ambiguity**: use a named mapping so V810 production fixture selection is explicit without treating live integration prefixes as the same contract.
