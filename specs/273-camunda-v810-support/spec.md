...
**Independent Test**: Configure each documented 8.10 alias, verify that every alias selects the same 8.10 compatibility line, match that configuration against an observed Camunda 8.10 gateway, and confirm that omitting the version still selects the pre-existing default.
...
3. **Given** an operator does not configure a Camunda version, **When** c8volt resolves its version, **Then** the existing default version remains unchanged.
...
As an operator using Camunda 8.7, 8.8, or 8.9, I want Camunda 8.10 support to be additive so that my current commands, defaults, generated runtime contracts, and integration coverage remain unchanged.

**Why this priority**: A new compatibility line must not regress currently supported production environments.

**Independent Test**: Run the stable-version regression suite and compare protected 8.7-8.9 runtime artifacts, default selection, command behavior, and integration assets before and after the feature.
...
4. **Given** no Camunda version is configured, **When** the feature is present, **Then** the selected default is identical to the default before the feature.
...
- **FR-006**: The system MUST match a configured 8.10 identity with an observed Camunda gateway version in the 8.10 release line and MUST reject a different or unrecognizable release line.
- **FR-007**: The system MUST keep the existing default Camunda version unchanged.
...
- **Camunda 8.10 Compatibility Identity**: The single operator-facing release identity, its canonical value, accepted aliases, and relationship to the unchanged default.
...
- **SC-002**: A configured 8.10 selection matches observed Camunda 8.10 gateway versions in all supported matching tests and rejects every tested different or unrecognizable release line.
...
- **SC-006**: The pre-existing default version and 100% of tested Camunda 8.7, 8.8, and 8.9 behaviors remain unchanged after 8.10 support is added.
...
- **SC-011**: At least 90% of evaluators using only the version output and documentation can correctly identify the configured identity, active upstream baseline, prerelease status, accepted aliases, and unchanged default on their first attempt.
