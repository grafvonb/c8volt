...
- Implement the design before using the new flag. The named `APIDiagnostics` tests below are planned acceptance tests, not existing passing tests at planning time.
...
The test fixtures should create fresh clients/configuration for each enabled/disabled run and provide deterministic response bodies and operation state. Assertions must observe the real transport, not replace the facade with a mock.

Expected proof:

- Successful, failed, timed-out, canceled and retried exchanges yield one record each at the observed boundary. Redirects and token requests share the same invocation sequence.
- A controlled server sends initial headers before delayed body bytes. First-response precedes total; use controlled synchronization and tolerances, not fragile exact sleep comparisons.
...
| Quiet | Existing quiet result behavior; explicit diagnostics still emitted |
...
Benchmarks compare disabled/enabled diagnostics using a discard writer, small responses and streamed large bodies. Report allocations and runtime; metadata memory must not grow with payload size. Blocking stderr behavior is documented, not hidden behind an unbounded queue.
...
./bin/c8volt --config ./config.yaml get pd --stat --api-diagnostics > /tmp/c8volt-diagnostics-results.txt 2> /tmp/c8volt-diagnostics-stderr.txt
./bin/c8volt --config ./config.yaml get pd --stat --quiet --json --api-diagnostics > /tmp/c8volt-diagnostics-results.json 2> /tmp/c8volt-diagnostics-quiet.txt
```

Use the actual local configuration path if different. Inspect the result and stderr files separately: only stderr contains `api:` records; quiet does not suppress them. A live changing dataset is unsuitable for byte-for-byte baseline comparisons; use the deterministic automated fixtures for that guarantee. Mutation proof comes from the local cancellation tests, not a live purge or resource creation workflow.
...
Inspect generated CLI documentation for the inherited flag and README guidance for timing, incomplete transfers, retained identifiers, redaction, quiet/stderr behavior and the observation boundary. Do not hand-edit generated CLI pages. Record test outcomes and any platform limitations before committing; full race-enabled `make test` is mandatory for implementation delivery.
