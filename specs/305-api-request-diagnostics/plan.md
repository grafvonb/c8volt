...
| CLI-first, script-safe | Pass | Existing verbose opt-in and configured logger; stdout and exit outcomes preserved; configured/inherited stderr and quiet combinations have execution tests. |
...
```text
cmd/
├── root.go                         # existing verbose help and effective stderr selection
...
1. **Bootstrap**: Reuse resolved verbose configuration and invocation logger. Resolve executing `cmd.ErrOrStderr()` before activity wrapping and create the normal configured logger with that writer. Pass the logger, verbose selection, profile/tenant and redaction inputs before authenticator initialization. No new flag or logging configuration. Install no diagnostic observer when verbose is off or the logger filters INFO.
2. **Service construction**: Add a `WithDiagnostics` option and helper in httpc that installs the observer under the current LogTransport only when enabled. One collector owns the sequence and retains the existing invocation logger. OAuth's plain token-client construction reuses this collector through a shared helper that locates it through known wrappers without copying auth/retry behavior. Do not mutate `http.DefaultTransport`, unrelated clients or global output state.
...
- **Logger compatibility**: Cover plain, plain-time, text and JSON logging, timestamp/source options, verbose off, debug-only, verbose plus quiet and restrictive configured levels. Required fields survive in message text across handlers; JSON logs remain separate from command JSON stdout.
