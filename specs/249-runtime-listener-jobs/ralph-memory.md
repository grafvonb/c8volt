...
- `cmd/walk_processinstance.go` now registers `--with-listeners`, validates that it requires `--with-elements` and rejects keys-only output before traversal enrichment, then routes walked element enrichment through `enrichProcessInstancesWithElementListenerActivityOptions` when listeners are requested.
- Walk listener output reuses the shared process-instance activity model and renderer: `activityTraversalPayload` preserves listener arrays in JSON, and `formatProcessInstanceElementTreeLines` keeps listener rows inside owning element blocks for family, children, parent, and flat modes.
...
- `cmd/ops_analyse_slow_process_instances.go` now registers `--with-listeners`, rejects keys-only listener output before client lookup, and maps `WithListeners` into `ops.SlowProcessAnalysisRequest`.
- `cmd/cmd_views_ops_slow_process_analysis.go` renders listener rows under element timeline rows in default hotspot and full-timeline modes. Transition rows never render listener children; JSON uses listener fields on element timeline entries only.
...
- `internal/services/ops/slow_process_analysis.go` switches slow-analysis element enrichment to `processinstance.EnrichProcessInstancesWithElementListeners` when `WithListeners` is requested, then copies element listener pointers onto element timeline rows. Default analysis still uses element-only enrichment and does not call the job service.
