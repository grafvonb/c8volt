...
## Current Handoff
- US3 `walk pi --with-elements --with-listeners` is complete and validated. T001-T003 remain unchecked because they are review-only. Next iteration should start US4 with slow-process analysis listener tests T040-T044, then wire `ops analyse slow-process-instances --with-listeners` through the ops facade/service and renderer.
