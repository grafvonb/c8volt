...
- [x] T005 [P] [US1] Add v810 native effective-variable contract tests in `internal/services/usertask/v810/variables_test.go` for route, task key, ascending name sort, offset-only page, `truncateValues=false`, raw values and both truncation fields, required metadata/value presence versus valid zero/empty values, preserved scope/tenant, and HTTP/malformed-response errors without fallback or recovery calls.
- [x] T006 [P] [US1] Add equivalent v89 adapter contract coverage in `internal/services/usertask/v89/variables_test.go`, asserting the matching generated request types, raw decoding, error categories, and zero extra calls rather than assuming v810 tests prove version support.
- [x] T007 [P] [US1] Add equivalent v88 adapter contract coverage in `internal/services/usertask/v88/variables_test.go`, including backend authorization for selected keys and unchanged actual scope/tenant fields.
- [x] T008 [P] [US1] Add v87 no-request unsupported-operation coverage in `internal/services/usertask/v87/variables_test.go`, retaining native-read and legacy resolver/fallback regression cases in `internal/services/usertask/v87/native_test.go`.
- [x] T009 [P] [US1] Add complete pagination/enrichment tests in `internal/services/usertask/variables_test.go`: exact/capped totals, retained highest lower bound, sparse pages including continuation beyond the cap, raw-count offset advancement, terminal empty pages, contradictory metadata, cancellation/overflow, identical duplicate collapse versus conflicting-name failure, stable name/task order, no process-root filtering, empty inputs, and later-page/task failures.
- [x] T010 [P] [US1] Add facade contract tests in `c8volt/task/variables_test.go` for option mapping, `ferrors` conversion, unchanged task/variable fields, no loops or re-fetches, and the invariants “Item unchanged; variables initialized even when empty” and “Total equals returned item count; initialized empty items; order matches input”, including nil input and `items: []`/`variables: []` JSON shapes.
- [x] T011 [P] [US1] Add keyed execution tests in `cmd/get_usertask_test.go` using T004 fixtures: all aliases, repeated/comma-separated and implicit/explicit stdin keys, merged/deduplicated order, strict mixed-found/missing input, authorized cross-discovery-tenant keys, denial/disappeared task/later-variable failures, no variables, and baseline human/JSON output with stdout/stderr separate and JSON decoder EOF.

### Implementation for User Story 1

- [x] T012 [US1] Implement v810 `SearchUserTaskEffectiveVariablesPage` in `internal/services/usertask/v810/variables.go` and extend `internal/services/usertask/v810/contract.go` with the matching generated call; apply shared response/error helpers, offset/name-sort requests and explicit false truncation, and raw decoding where “Required `value` presence is distinguishable from a valid empty string”; validate required total/capped metadata and retain continuation evidence; update affected local client doubles and pass T005.
- [x] T013 [P] [US1] After T012, implement the explicit v89 counterpart in `internal/services/usertask/v89/variables.go` and `internal/services/usertask/v89/contract.go`, update affected v89 client doubles, preserve raw-field/truncation/metadata semantics, and pass T006.
- [x] T014 [P] [US1] After T012, implement the explicit v88 counterpart in `internal/services/usertask/v88/variables.go` and `internal/services/usertask/v88/contract.go`, update affected v88 client doubles, preserve raw-field/truncation/metadata semantics, and pass T007.
- [x] T015 [P] [US1] After T012, add the established unsupported implementation in `internal/services/usertask/v87/variables.go` and signature in `internal/services/usertask/v87/contract.go`, keeping legacy resolver behavior intact and passing T008 without native variable requests.
- [x] T016 [US1] After T012–T015, extend `internal/services/usertask/api.go` and implement complete offset retrieval plus sequential `EnrichUserTasksWithVariables` in `internal/services/usertask/variables.go`; follow all seven pagination rules in `specs/309-user-task-variables/contracts/facade-service.md`, retaining lower bounds/continuation evidence and checking cancellation/overflow; initialize empties, preserve raw counts, sort/collapse only identical records, reject conflicts, propagate errors, update affected service stubs/assertions, and pass T009.
- [x] T017 [US1] Add the single public `EnrichUserTasksWithVariables` operation to `c8volt/task/api.go` and `c8volt/task/client.go` with mechanical mappings in `c8volt/task/convert.go`; delegate to T016, map options/errors, preserve empty arrays and returned-count semantics, update affected facade stubs/assertions, and pass T010 without altering constructors or existing lookup/search/resolver APIs.
- [x] T018 [US1] Extract a narrow explicit-limit variable formatter into `cmd/cmd_views_variable_values.go`, retaining the PI wrapper in `cmd/cmd_views_processinstance_vars.go`; add baseline task `vars:` tree and enriched JSON views in `cmd/cmd_views_usertask_vars.go` using existing tree/row helpers, default unlimited values, backend truncation labels, `items: []`/`variables: []`, no process-age metadata, and propagated writer errors; preserve existing PI formatter tests in `cmd/cmd_views_processinstance_test.go`.
- [x] T019 [US1] Register `--with-vars` in `cmd/get_usertask.go` and add focused selected-result dispatch in `cmd/get_usertask_vars.go`; retain strict keyed lookup before one enrichment pass, gate absent flag/effective keys-only/count/empty inputs without variable calls, honor JSON precedence and quiet/error semantics, keep the flag outside search-selector conflicts, reset new flag state in command test setup, and pass T011.
- [x] T020 [US1] Format touched Go files and run focused adapter/service/facade/keyed-command and PI formatter checks from `specs/309-user-task-variables/quickstart.md`; verify T005–T011 pass and record actual commands/outcomes in the execution notes of `specs/309-user-task-variables/tasks.md`, including any limits on validation rather than claiming unchecked success.
...
- [x] T024 [P] [US2] Extend `cmd/get_usertask_error_test.go` for enrichment failure after an earlier streamed page, collected-search failure, and writer failure; verify existing exit/error conventions, no final successful summary/envelope, no empty substitute, and unchanged task-search failures.

### Implementation for User Story 2

- [x] T025 [US2] Integrate enrichment in `cmd/get_usertask_search.go` only after service trimming of `step.Page.Items`, using `cmd/get_usertask_vars.go` before incremental render/prompt; enrich the final selected collection once for collected modes, skip empty pages, retain summary-only completion without re-fetching, and preserve existing visitor decisions, stderr prompts, filters/tenants, and service-owned traversal while passing T021–T024.
- [x] T026 [US2] Run focused search/output/error command tests and `TestGetUserTaskPagingTerminal` from `specs/309-user-task-variables/quickstart.md`; verify request counts, stdout/stderr purity, supported-version execution, and unchanged keyed regression behavior, and record evidence in `specs/309-user-task-variables/tasks.md`.
...
- 2026-09-15, iteration 5 (T005, T012): Added v8.10 contract coverage and the native effective-variable page adapter. The red check failed because `SearchUserTaskEffectiveVariablesPage` was absent; after implementation, `go test ./internal/services/usertask/v810 -run '^TestService_SearchUserTaskEffectiveVariablesPage' -count=1`, the complete v8.10 package, the focused race check, `go test ./internal/services/usertask/... -run 'Test.*(Variable|Native|Resolve)' -count=1`, and `git diff --check` passed. Tests cover the native route/key, offset-only body, ascending name sort, `truncateValues=false`, raw values and truncation precedence, valid empty required fields, actual scope/tenant preservation, capped/exact metadata, continuation evidence, HTTP/transport errors, and malformed raw payloads without fallback or recovery calls.
- 2026-09-15, iteration 7 (T007, T014): Added v8.8 contract coverage and the explicit native effective-variable adapter. The red check failed because `SearchUserTaskEffectiveVariablesPage` was absent; after implementation, `go test ./internal/services/usertask/v88 -run '^TestService_SearchUserTaskEffectiveVariablesPage' -race -count=1`, the complete v8.8 package, `go test ./internal/services/usertask/... -run 'Test.*(Variable|Native|Resolve)' -count=1`, and `git diff --check` passed. Coverage verifies selected-key backend authorization, offset-only/name-sorted requests, `truncateValues=false`, unchanged actual scope/tenant metadata, raw value/truncation semantics, required-field validation, continuation evidence, and transport/HTTP/malformed failures without fallback calls.
- 2026-09-15, iteration 8 (T008, T015): Added v8.7 no-request unsupported-operation coverage and the explicit compatibility implementation. The red check failed because `SearchUserTaskEffectiveVariablesPage` was absent; after implementation, the focused variable test, native/read/search resolver regressions, complete v8.7 race package, cross-version user-task variable/native/resolver tests, and `git diff --check` passed. The v8.7 adapter returns the established domain unsupported error without issuing a transport request or changing legacy behavior.
- 2026-09-15, iteration 9 (T009, T016): Added complete service pagination/enrichment coverage and the version-neutral offset traversal plus sequential attachment workflow. The red check failed because `SearchUserTaskEffectiveVariables` and `EnrichUserTasksWithVariables` were absent; after implementation, `go test ./internal/services/usertask -run 'Test(SearchUserTaskEffectiveVariables|EnrichUserTasksWithVariables)' -race -count=1`, `go test ./internal/services/usertask/... -run 'Test.*(Variable|Native|Resolve)' -race -count=1`, `go test ./internal/services/usertask/... -count=1`, `go test ./c8volt/task -count=1`, repository-wide compile-only `go test ./... -run '^$' -count=1`, and `git diff --check` passed. Coverage proves exact/capped sparse paging, retained lower bounds and continuation evidence, raw-count offsets, terminal empties, contradictions, cancellation/overflow, duplicate integrity, ordering/local scopes, empty inputs, and later page/task failures.
- 2026-09-15, iteration 11 (T011, T018–T020): Added keyed `--with-vars` execution coverage, the explicit-limit shared formatter, enriched task human/JSON views, and focused keyed dispatch. The red command check failed on the absent `--with-vars` flag; after implementation, `go test ./cmd -run '^TestGetUserTaskCommand_KeyedVariable' -count=1`, `go test ./cmd -run 'Test(GetUserTask|UserTasksView)' -count=1`, `go test ./cmd -run '^TestGetUserTask' -race -count=1`, `go test ./internal/services/usertask/... -run 'Test.*(Variable|Native|Resolve)' -race -count=1`, `go test ./c8volt/task -run 'Test.*(Variable|Enrich|Convert)' -race -count=1`, focused task/PI formatter race tests, and `git diff --check` passed. Coverage includes all aliases, repeated/comma/stdin merging, complete pages and duplicate normalization, actual scope/tenant metadata, no-variable tasks, strict read/page failures, stdout/stderr separation, JSON EOF, default unlimited formatting, writer errors, and zero variable requests in excluded keyed/count/empty modes. No live Camunda check or full repository suite was run; those remain deferred to the integrated validation tasks.
- 2026-09-15, iteration 12 (T021–T026): Added bounded-search variable coverage and integrated enrichment after service selection. The red check failed because search pages and collected results remained unenriched; after implementation, the focused US2 search/output/error suite, the complete real-terminal paging matrix, `go test ./cmd -run '^TestGetUserTask' -race -count=1`, a focused race rerun for search/output/error cases, and `git diff --check` passed. Coverage proves partial-page limits, sparse incremental/collected traversal, complete independent variable paging, tenant/filter preservation, zero excluded/empty/keys/count reads, prompt-stream and accepted-page boundaries, unattended modes, no duplicate final enrichment, later enrichment failures, writer errors, and unchanged keyed behavior across supported versions. No live Camunda check or full repository suite was run; those remain deferred to the integrated validation tasks.
- 2026-09-15, iteration 13 (T027–T032): Added the task-specific `--var-value-limit`, explicit-limit view plumbing, execution/view regressions, and final US3 help/capability metadata. The red check failed because the task limit flag/state did not exist; after implementation, `go test ./cmd -run 'Test(GetUserTask|UserTasksView)' -count=1`, `go test ./cmd -run 'Test.*ProcessInstance.*(Variable|Vars)' -count=1`, `go test ./cmd -run '^TestCommandCapabilityForCommand_UserTaskReadContract$' -count=1`, the focused US3 race suite, and `git diff --check` passed. Coverage proves default/explicit-zero/positive Unicode limits after compaction, backend/client/both truncation labels, JSON EOF and unshortened values, JSON-over-keys and quiet precedence, zero excluded-mode requests, pre-request dangling/negative validation, retrieval failure propagation, PI flag independence, and all four issue examples. No live Camunda check or full repository suite was run; those remain deferred to T033–T036.
- 2026-09-15, iteration 14 (T033–T036): Updated README and quickstart guidance, regenerated CLI documentation with `make docs-content`, and reviewed the feature diff against both contracts. The generated user-task page and dependent index contain both flags and all four workflows; no generated Camunda client changed. Static inspection confirmed variable paging remains in `internal/services/usertask`, facade mapping remains mechanical, command/view files retain cohesive ownership, interface doubles compile, legacy resolver and tenant behavior remain covered, and no mutation or out-of-scope filtering was added. All focused quickstart suites and `TestGetUserTaskPagingTerminal` passed. `gofmt` and `git diff --check` passed. The first `make test` attempt hit the default 10-minute `cmd` package timeout while an unrelated diagnostics subprocess test was active; that test passed alone under race, and `GOFLAGS='-timeout=20m' make test` then passed every package (`cmd` completed in 612.012s). No live Camunda check was run because it requires operator credentials and existing authorized tasks; HTTP fixtures cover the specified backend/error cases without mutations.
