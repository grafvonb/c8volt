...
- Q: How should relative `--end-*-days` filters treat process instances that do not have an `endDate`? → A: Exclude instances with no `endDate` whenever `--end-before-days` or `--end-after-days` is used.
...
1. **Given** a v8.8 environment with process instances across multiple recent and older days, **When** the user runs `c8volt get process-instance --start-after-days 7`, **Then** only process instances whose start date is on or after the derived day boundary for seven days ago are returned.
2. **Given** a v8.8 environment with process instances across multiple recent and older days, **When** the user runs `c8volt get process-instance --start-after-days 30 --start-before-days 7`, **Then** only process instances whose start date falls within the inclusive derived day range are returned.
3. **Given** a v8.8 environment with completed process instances that ended on different days, **When** the user runs `c8volt get process-instance --end-before-days 14`, **Then** only process instances whose end date is on or before the derived boundary for fourteen days ago are returned.
...
1. **Given** a v8.8 environment with active process instances started on different days, **When** the user runs `c8volt cancel process-instance --state active --start-before-days 30`, **Then** only active process instances whose start date is on or before the derived thirty-day boundary are selected for cancellation.
2. **Given** a v8.8 environment with completed process instances ending on different days, **When** the user runs `c8volt delete process-instance --end-after-days 60 --end-before-days 7 --auto-confirm`, **Then** only process instances whose end date falls within that inclusive derived day range are selected for deletion.
...
2. **Given** a user mixes `--start-after-days` with `--start-date-after` or any other relative and absolute flag pair for the same field, **When** the command is executed, **Then** the command fails with a clear validation error instead of choosing one input silently.
...
- A process instance with no `endDate` must be excluded whenever `--end-before-days` or `--end-after-days` is used.
...
- **FR-001**: The system MUST extend `c8volt get process-instance` with `--start-before-days`, `--start-after-days`, `--end-before-days`, and `--end-after-days`.
- **FR-002**: The system MUST extend `c8volt cancel process-instance` with `--start-before-days`, `--start-after-days`, `--end-before-days`, and `--end-after-days`.
- **FR-003**: The system MUST extend `c8volt delete process-instance` with `--start-before-days`, `--start-after-days`, `--end-before-days`, and `--end-after-days`.
...
- **FR-005**: The system MUST treat `--start-before-days N` as an inclusive upper bound derived from the local calendar day for today minus `N` days.
- **FR-006**: The system MUST treat `--start-after-days N` as an inclusive lower bound derived from the local calendar day for today minus `N` days.
- **FR-007**: The system MUST treat `--end-before-days N` as an inclusive upper bound derived from the local calendar day for today minus `N` days.
- **FR-008**: The system MUST treat `--end-after-days N` as an inclusive lower bound derived from the local calendar day for today minus `N` days.
...
- **FR-016**: On v8.8, the system MUST exclude process instances with no `endDate` whenever one or more `--end-before-days` or `--end-after-days` filters are supplied.
