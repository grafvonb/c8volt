...
**Independent Test**: Run `c8volt get process-instance --start-date-older-days 7`, `--start-date-newer-days 30`, and `--end-date-newer-days 14` against a v8.8 config and verify the resulting search request behaves like the equivalent derived absolute date filters, including inclusive boundaries.
...
**Independent Test**: Run `c8volt cancel process-instance --state active --start-date-newer-days 30` and `c8volt delete process-instance --end-date-older-days 60 --end-date-newer-days 7 --auto-confirm` on v8.8 and verify only matching instances are selected through the existing search-based management path.
