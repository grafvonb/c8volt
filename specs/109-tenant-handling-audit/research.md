...
- [`cmd/walk_processinstance.go`](/Users/adam.boczek/Development/Workspace/Boczek/Projects/c8volt/c8volt/cmd/walk_processinstance.go) routes `--parent`, `--children`, and `--family` through `cli.Ancestry(...)`, `cli.Descendants(...)`, and `cli.Family(...)`. Those flows mix direct lookup and child expansion through the shared walker and are the primary traversal-leak surface.
