...
## Current Handoff
- Next iteration starts Phase 6 / US4 at task T036: add capability JSON and human tests for additive `allTenantsSupport` values and unchanged document version `v1` in `cmd/command_contract_test.go` and `cmd/capabilities_test.go`; use the existing all-tenants support resolver as the source of truth.
