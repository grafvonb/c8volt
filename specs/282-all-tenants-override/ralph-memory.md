...
## Current Handoff
- Next iteration stays in Phase 3 / US1 at task T015: add durable-progress tests proving the all-tenants warning appears once before effective scope without leaking to protected output in `cmd/processinstance_mutation_progress_test.go` and `cmd/ops_progress_test.go`; T019 remains open until all US1 tests are ready to run together.
