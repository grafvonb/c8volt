...
- [ ] T011 [P] [US1] Add failing base/profile/environment, already-empty, explicit-false, absent-flag, and Camunda 8.7 post-normalization override tests in `cmd/root_config_test.go`
- [ ] T012 [P] [US1] Add failing exact-warning, configured-tenant ordering, once-only, already-unfiltered, and existing `--tenant ""` regression tests in `cmd/cmd_views_tenant_context_test.go`
...
- [ ] T016 [US1] Extend private tenant override provenance with an all-tenants origin and exact broadening warning while preserving public `tenant.Context` in `cmd/cmd_tenant_context.go`
- [ ] T017 [US1] Apply active all-tenants after `retrieveAndNormalizeConfig` by capturing the resolved configured tenant and setting the effective tenant to empty in `cmd/root_config.go`
- [ ] T018 [US1] Invoke the post-normalization override before configuration enters command context or service installation in `cmd/root.go`
- [ ] T019 [US1] Run focused US1 tests for `cmd/root_config_test.go`, `cmd/cmd_views_tenant_context_test.go`, `cmd/config_test.go`, `cmd/get_processinstance_test.go`, `cmd/get_test.go`, `cmd/processinstance_mutation_progress_test.go`, and `cmd/ops_progress_test.go`
