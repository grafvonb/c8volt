...
- [x] T069 Add explicit-large-work progress for `walk process-instance`, bulk `run process-instance`, and `ops execute smoke-test` in `cmd/walk_processinstance.go`, `cmd/run_processinstance.go`, `cmd/ops_execute_smoketest.go`, `c8volt/process`, `c8volt/ops`, and matching service tests
...
- [x] T053 Run focused activity tests with `GOCACHE=/tmp/c8volt-gocache go test ./toolx/logging ./testx/activitysink -count=1` for `toolx/logging/activity.go` and `testx/activitysink/activity_sink.go`
