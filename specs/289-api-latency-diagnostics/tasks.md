...
- [x] T039 [P] [US3] Add failing command tests for scoped interrupt cancellation, cleanup continuation, retained output, partial report attempts, exact recovery guidance, established nonzero error envelope, and no-cleanup confirmation in `cmd/ops_execute_api_latency_test.go`
...
- [x] T049 [US4] Add failing output-safety and progress tests covering tokens, authorization headers, secrets, variables, payloads, raw response bodies, unbounded errors, JSON/automation silence, quiet failures, and verbose/debug detail in `cmd/ops_analyse_api_latency_test.go` and `cmd/ops_execute_api_latency_test.go`
- [x] T050 [US4] Add failing subprocess tests proving completed abnormal evidence exits successfully while invalid, incomplete, report-failed, and requested-cleanup-failed runs use the established nonzero error envelope in `cmd/ops_analyse_api_latency_test.go` and `cmd/ops_execute_api_latency_test.go`
...
- [x] T060 [P] Add command discoverability, safety distinction, and representative examples to `README.md` and `docs/ops/index.md`
- [x] T061 [P] Add focused operator guidance in `docs/ops/analyse-api-latency.md` and `docs/ops/execute-api-latency-test.md`
...
## Phase 8: Convergence - Deterministic Active Cleanup

**Purpose**: Close the real Camunda 8.9 cleanup regression discovered after the original implementation without adding a new contract or configuration surface.
