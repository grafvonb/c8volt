...
- [x] T037 [P] [US4] Add confirmed deletion command test for exact frozen-plan root submission in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T038 [P] [US4] Add execution-control mapping tests for workers, fail-fast, no-worker-limit, no-wait, and force in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T039 [P] [US4] Add `--automation --json` without `--auto-confirm` success test for supported state-changing incident purge in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T040 [P] [US4] Add local-precondition failure subprocess tests for post-planning blockers and exit code in `cmd/ops_purge_processinstances_with_incidents_test.go`

### Implementation for User Story 4

- [x] T041 [US4] Execute deletion through existing process-instance deletion service from `internal/services/ops/incident_purge.go`
- [x] T042 [US4] Use `shouldImplicitlyConfirm(cmd)` for destructive confirmation decisions in `cmd/ops_purge_processinstances_with_incidents.go`
- [x] T043 [US4] Preserve no-wait, confirmation, per-key or per-batch status, and final outcome in `internal/domain/ops_incident_purge.go`
- [x] T044 [US4] Render deletion execution and final outcome in `cmd/cmd_views_ops_purge_processinstances_with_incidents.go`
- [x] T045 [US4] Mark US4 tasks complete and record validation notes in `specs/199-ops-incident-purge/progress.md`
...
- [x] T046 [P] [US5] Add verbose key-list output tests for incident, candidate, root, affected, and skipped keys in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T047 [P] [US5] Add deterministic `--dry-run --json` and `--automation --json` output tests in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T048 [P] [US5] Add Markdown incident purge report rendering test in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T049 [P] [US5] Add JSON incident purge report rendering test in `cmd/ops_purge_processinstances_with_incidents_test.go`
- [x] T050 [P] [US5] Add existing report-file preservation tests for dry-run, unconfirmed, and locally blocked runs in `cmd/ops_purge_processinstances_with_incidents_test.go`

### Implementation for User Story 5

- [x] T051 [US5] Reuse shared ops report-file validation, format inference, overwrite safety, and file writing in `cmd/ops_purge_processinstances_with_incidents.go`
- [x] T052 [US5] Extend report model/rendering for incident discovery, candidate set, plan, deletion, notices, errors, and outcome fields in `cmd/cmd_views_ops_purge_processinstances_with_incidents.go`
- [x] T053 [US5] Keep normal human output compact and gate detailed key lists behind verbose output in `cmd/cmd_views_ops_purge_processinstances_with_incidents.go`
- [x] T054 [US5] Print compact `report: written <path>` human output after report writes in `cmd/cmd_views_ops_purge_processinstances_with_incidents.go`
- [x] T055 [US5] Mark US5 tasks complete and record validation notes in `specs/199-ops-incident-purge/progress.md`
