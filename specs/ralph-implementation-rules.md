...
- Keep detailed functional service logs available under `--verbose`. Low-level HTTP exchange diagnostics use the existing DEBUG logger (`--debug` or configured debug level), independently of verbose. Do not delete diagnostics that are useful for support; route them behind the existing verbose and suppression mechanisms.
