...
**Constraints**: Preserve existing `get process-instance` flag semantics and validation, keep `--orphan-children-only` client-side, apply pushdown only where the active version has a reliable request-side representation, audit other relevant `get` command families and record bounded no-addition rationale if none qualify, reuse the existing versioned service/facade architecture, update source and regenerate only if generated artifacts actually need to change, and finish with `make test`  
...
| Other `get` command families | No equivalent server-capable post-fetch filter seam confirmed yet during planning scan | Task the audit explicitly and record no-addition rationale if implementation confirms no other qualifying filters |
...
5. Audit the rest of the `get` command surface for equivalent late-filtering seams, documenting explicit no-addition rationale if no other qualifying cases exist.
