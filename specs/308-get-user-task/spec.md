...
1. **Given** returned tasks, including unassigned tasks and tasks without a display name, **When** human output is selected, **Then** compact rows show task key, state, name or element ID, assignee, process-instance key, and tenant using established missing-value conventions.
...
- **FR-011**: Human output MUST use compact rows showing task key, state, name or element ID, assignee, process-instance key, and tenant. Default output MUST avoid per-request and per-page diagnostic details.
...
- Planning will retain the issue's required command execution and real-terminal checks, documentation regeneration with `make docs-content`, formatting of touched Go files, targeted tests, and the constitution's full `make test` requirement before implementation acceptance.
