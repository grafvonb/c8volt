...
- **FR-011**: Human output MUST follow get-command ordering: task key, tenant, element ID, state, related `pi:`/`ei:`/`pd:` keys. Assignee MUST be the last human field and show `assignee:<unassigned>` when empty. Name, BPMN process ID, and process-definition version are JSON-only fields; empty optional fields are omitted. Default output MUST avoid per-request and per-page diagnostic details.
