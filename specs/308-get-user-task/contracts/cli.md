...
Human rows use existing flat-row alignment without headings. Column order is task key, state, name (element ID fallback), assignee, `pi:<process-instance-key>`, tenant using nearby tenant-cell conventions. Empty optional cells follow the existing omission/alignment behavior; no new placeholder policy is introduced. The list summary is `found: N` with a trailing newline. Completed empty search emits exactly `found: 0\n`.
