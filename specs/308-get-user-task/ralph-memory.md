...
- User-task command output uses one collection payload for every keyed cardinality, JSON-before-keys-before-human precedence, quiet suppression only for human output, aligned get-command rows ordered key/tenant/element ID/state, with optional name followed by BPMN ID and pi/ei/pd labels, with assignee always last and `<unassigned>` when empty, and writer errors propagated from human/key lines.
