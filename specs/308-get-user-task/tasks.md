...
- [x] T013 [US1] Create `cmd/cmd_views_usertask.go` with flat rows in contract column order, technical IDs plus labelled name/assignee and pi/ei/pd keys, omitted empty optional fields, returned-count collection envelope, keys-only lines, and `found: N` summary; choose JSON before keys before human and then suppress quiet human output, preserving the one-key collection shape; add focused writer-error/row tests in `cmd/cmd_views_usertask_test.go`.
