...
- **Operational Proof Over Intent**: Pass. The command is read-only, and tests must prove actual list, lookup, filter, invalid-combination, JSON, sorting, and unsupported-version outcomes rather than merely wiring help text.
...
- Confirm `--key` and `--filter` are mutually exclusive because filtering is list-only and keyed lookup must not be silently broadened, narrowed, or altered.
...
- Add `cmd/get_tenant.go` with `--key` and `--filter`, command aliases only if they match existing naming conventions, automation support, read-only mutation classification, JSON support, and validation that `--key` and `--filter` are rejected together as an invalid flag combination.
...
5. Add User Story 4 JSON, invalid-combination handling for `--key` plus `--filter`, unsupported-version coverage, generated docs, and final validation.
...
- **CLI-First, Script-Safe Interfaces**: Still passes. The command uses existing output modes and global flags, rejects invalid flag combinations deterministically, and avoids a new filtering language.
