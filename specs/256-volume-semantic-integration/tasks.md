...
- [ ] T032 [P] [US4] Implement `TestVolumeOpsExecuteFamily` for preview/confirmed execution reporting, step status vocabulary, mutation accounting, no-wait/submitted distinctions, and `volume-ops-reports-execute.json` evidence in `integration/cli/volume_ops_execute_test.go` (partial: smoke-test dry-run/confirmed no-wait cleanup and retention dry-run/confirmed no-match target is live; aged retention deletion, explicit pd-version/tag, and incident/job-state smoke paths remain)
- [ ] T033 [P] [US4] Implement `TestVolumeOpsPurgeFamily` for destructive purge preview/confirmed reporting, preserve/overwrite report path behavior, retained resources, cleanup-failed records, and `volume-ops-reports-purge.json` evidence in `integration/cli/volume_ops_purge_test.go` (partial: all-PDs/orphan/incident purge dry-run and confirmed no-match report target is live; true orphan, incident deletion, overwrite/preserve conflict, and cleanup-failed records remain)
- [ ] T034 [P] [US4] Implement `TestVolumeOpsRepairFamily` for repair preview/confirmed reporting, worker/limit accounting, partial failures, notices/errors, and `volume-ops-reports-repair.json` evidence in `integration/cli/volume_ops_repair_test.go` (partial: incident and process-instance preview plus actual no-wait repair target is live; deterministic partial failures, stale incidents, and mixed related-job notices remain)
