...
- `cmd/cmd_views_rendermode.go` is the existing shared output-mode seam. `pickMode()` maps global flags to `json`, `keys-only`, and the default `one-line` mode, while `itemView(...)` and `listOrJSON(...)` already centralize the JSON vs. line-oriented branching for many read flows.
- `cmd/cmd_views_get.go` shows the current repository-native JSON wrapping pattern: most `get` flows reuse public payload models directly, but some JSON responses already add command-local metadata wrappers such as the process-instance age JSON meta structure. That makes `cmd/` the right place to add a shared machine envelope without replacing public payload types.
...
- `cmd/walk_processinstance.go` is the custom traversal-rendering outlier. It switches among ancestry, descendants, default family tree, and flat family views through command-local render functions.
