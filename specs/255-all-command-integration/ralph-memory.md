...
- US5 proposal reports are legacy aggregate harness evidence: `TestProposalReports` writes `proposals-command.json` and `proposals-embedded-bpmn.json` from per-family append functions in `walk_test.go`, `update_test.go`, `ops_analyse_test.go`, and `ops_execute_test.go`. New integration work should follow `specs/integration-test-responsibility.md` and keep missing setup/fixture needs in spec-owned gap artifacts.
