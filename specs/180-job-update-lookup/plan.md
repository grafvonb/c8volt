...
**Primary Dependencies**: Cobra command tree, existing command metadata helpers, generated Camunda v8.8/v8.9 clients, versioned internal service factories, dedicated job facade package, shared rendering/error helpers, existing dry-run/confirmation command helpers, command context logger/activity plumbing, waiter/backoff helpers for retry confirmation
**Storage**: N/A  
...
**Constraints**: Preserve existing `get pi --with-incidents` and current `update pi --vars` planning/dry-run behavior; require `--key`; require at least one update flag; reject invalid retry/timeout values before calling Camunda; reject unsupported 8.7 job mutation before mutation; do not expose `retryBackOff`; do not confirm timeout through deadline comparison; compact plan output; no mutation during `--dry-run`; reject `--json --verbose` for `update job`; require auto-confirm or automation for non-dry-run JSON mutations; keep job behavior out of process-instance and incident services
**Scale/Scope**: One new `get job` command, one new `update job` command under the existing update root, a new job facade/domain/service path, versioned service implementations, job update plan aggregate, dry-run/no-op/confirmation gate behavior, waiter behavior for retries confirmation, human/JSON rendering, command metadata, tests, README/help/generated docs, and Speckit/Ralph artifacts
...
- **Operational Proof Over Intent**: PASS. The contract distinguishes planned, no-op, submitted, confirmed, confirmation-failed, and mutation-failed outcomes, and explicitly limits confirmation claims to retry checks where an observable reliable predicate exists.
- **CLI-First, Script-Safe Interfaces**: PASS. The contract defines valid/invalid invocations, dry-run behavior, JSON behavior, human output, required flags, no-wait behavior, and unsupported-version outcomes.
