...
| `camunda-88-mainstream-stability-validation.md` | Runs a real Camunda 8.8 stability validation using `config.yaml` profile `kind-camunda-platform-local-c88`, including exhaustive nested `ops` command coverage, private C88 substitutions, and no docs/help/example edits. | Ask Codex: `Run the prompt from specs/prompts/camunda-88-mainstream-stability-validation.md`. Make sure the local C88 cluster is running and reachable through the required profile first. |
| `release-docs-help-example-validation.md` | Audits README, docs, generated references, all command-help examples, and every nested `ops` command on Camunda 8.9. Defaults to report-only; repairs require an explicit request. Supports README-only scope and excludes VHS entirely. | Supply the config path, profile, expected cluster, disposable fixture scope, and private report directory, then ask Codex to run the prompt. Mutations require explicit authorization for the disposable scope. |
