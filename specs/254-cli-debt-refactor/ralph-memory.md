...
- Convergence T076 removed the remaining command-local element page item trim from `cmd/get_element_search.go`; `searchElementsWithPaging` now renders, reports progress, and prompts from the service-supplied `SearchPageStep.Page.Items`. `cmd/get_element_test.go` pins the boundary with a fake `SearchElementsPages` visitor page so service-owned limit trimming remains the sole production owner.
