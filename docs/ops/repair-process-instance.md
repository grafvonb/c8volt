...
Selector-based repair reports discovery tenant context, using `Tenant filter: tenant-a` for named discovery and `Tenant filter: none — resources from multiple tenants may be affected` when discovery is unfiltered. Explicit process-instance keys report `Tenant filter: not applied for explicit resource keys`, then include known process-instance or incident tenant evidence from the frozen repair plan. Cross-tenant and unknown-metadata warnings are non-blocking safety signals. JSON reports carry `tenantContext`; quiet and keys-only behavior remains protected for automation.
