...
Orphan purge uses discovery tenant semantics. A named tenant configuration limits discovery and is reported as `Tenant filter: tenant-a`; an empty tenant is reported as `Tenant filter: none — resources from multiple tenants may be affected`. The frozen delete plan can also show known resource tenants and non-blocking warnings when the affected set spans tenants or contains targets with unknown tenant metadata. JSON reports include the shared `tenantContext` object, and protected output modes keep stdout script-safe.
