...
Incident-filtered purge reports the tenant meaning used for discovery. Named configuration appears as `selection scope: tenant-a only`; an empty tenant appears as `selection scope: unfiltered across accessible tenants`. Explicit tenant flag changes are reported before scope, and clearing a named configuration with `--tenant ""` warns that selection is unfiltered. Direct incident keys use explicit-key semantics instead, so the configured tenant is reported as not applied and resource tenant evidence is shown only when already present in the frozen plan. Multi-tenant plans emit one warning-level `affected tenants: ...` summary, unknown-metadata warnings remain non-blocking, and both are represented in JSON reports through `tenantContext`; quiet and keys-only output stay free of tenant commentary on stdout.
