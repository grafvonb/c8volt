...
Retention is discovery-driven. A named tenant configuration is shown as `selection scope: tenant-a only`; an empty tenant configuration is shown as `selection scope: unfiltered across accessible tenants` and is not described as `<default>`. Explicit tenant flag changes are reported before scope, and clearing a named configuration with `--tenant ""` warns that selection is unfiltered. The frozen delete plan reports one known tenant informationally, emits one warning-level `affected tenants: ...` summary for multi-tenant scope, and warns separately for unknown tenant metadata. JSON output and JSON reports carry the same `tenantContext` object, while quiet mode and keys-only pipelines keep tenant guidance out of stdout.
