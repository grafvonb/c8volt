...
> Generated from build `c8volt v4.3.0-beta.1-316-gb459ce07-dirty`, commit `b459ce07`, built `2026-09-15T05:50:18Z` | Supported Camunda 8 versions: 8.7, 8.8, 8.9, 8.10 | Camunda 8.10 baseline: 8.10.0-alpha4 (prerelease)
...
Human rows follow the other get commands: task key, tenant, element ID, and state, followed by optional `name:` details, then BPMN process ID and related `pi:`, `ei:`, and `pd:` keys. Assignee is always last: `assignee:<user>` when assigned, otherwise `assignee:<unassigned>`. Other empty optional fields are omitted; collections end with `found: N`.
