...
func TestOpsExecuteAPILatencyDryRunJSONPlansWithoutMutation(t *testing.T) {
	...
	require.Equal(t, float64(19), plan["visibilityAttemptLimit"])
	require.Equal(t, float64(140), plan["derivedRequestLimit"])
...
func TestRenderOpsExecuteAPILatencyHumanRendersActiveResult(t *testing.T) {
	...
	require.Contains(t, output, "scope: 1 test process instance; load 1 worker")
	require.Contains(t, output, "result: API measurements completed; request errors 0; timeouts 0")
	require.Contains(t, output, "create latency: 95% completed within 20ms at 1 worker")
	require.Contains(t, output, "search visibility: the test process instance became searchable; no visibility retries; slowest wait 750ms")
	require.Contains(t, output, "finding: no abnormal evidence found")
	require.Contains(t, output, "next: compare with a read-only diagnostic if symptoms continue")
	require.Contains(t, output, "cleanup: deleted all 2 run-owned resources (1 process instance, 1 process definition)")
	...
	require.NotContains(t, output, "run:")
	require.NotContains(t, output, "fixture:")
	require.NotContains(t, output, "ownership:")
...
func TestRenderOpsExecuteAPILatencyStableHumanAndJSON(t *testing.T) {
	...
	require.Contains(t, human, "search visibility: 1/2 test process instances became searchable; 2 checks found no matching instance; slowest wait 1.5s")
	require.Less(t, strings.Index(human, "finding: backpressure evidence"), strings.Index(human, "finding: delayed search visibility"))
	require.Contains(t, human, "next: inspect broker resource usage")
	require.Contains(t, human, "next: compare exporter lag")
	...
	require.NotContains(t, human, "run:")
	require.NotContains(t, human, "fixture:")
	...
	require.Contains(t, verboseOut.String(), "stage 1: workers 1; primary 1/1; derived 3/3")
	require.Contains(t, verboseOut.String(), "ownership: process definition recorded; process instances 3")
	require.Contains(t, verboseOut.String(), "notice: fixture deployment and cleanup are reported outside primary samples")
	require.Contains(t, verboseOut.String(), "limitation: bounded evidence does not prove capacity")
	flagVerbose = false
