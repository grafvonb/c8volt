...
func TestUserTasksView_RendersContractModes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "human", want: "2251799815391233 tenant-a approve_invoice CREATED   name:Approve invoice invoice pi:2251799813711967 ei:2251799815391200 pd:2251799813689000 assignee:alice\n2251799815391234 tenant-b archive_invoice COMPLETED                      invoice pi:2251799813711968 ei:2251799815391200 pd:2251799813689000 assignee:<unassigned>\nfound: 2\n"},
...
func TestFlatRowUserTask_UsesGetGrammar(t *testing.T) {
	...
	require.Equal(t, "2251799813900041 tenant-a SimpleUserTask_UserTask CREATED name:Simple User Task C89_SimpleUserTask pi:2251799813900036 ei:2251799813900040 pd:2251799813873873 assignee:<unassigned>", compactFlatRow(flatRowUserTask(item)))
}
