...
var cancelProcessInstanceCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("initializing client: %w", err))
		}
		if cmd.Flags().Changed("workers") && flagCancelPIWorkers < 1 {
			ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("--workers must be positive integer"))
		}
		...
		default:
			...
			if !ok {
				ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("either at least one --key is required, or sufficient filtering options to search for process instances to cancel"))
			}
			if searchFilterOpts.State.In(process.StateCanceled, process.StateCompleted, process.StateTerminated) {
				ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("it does not make sense to cancel process instances already in state %q", searchFilterOpts.State.String()))
			}
		...
		if len(keys) == 0 {
			ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("no process instance keys provided or found to cancel"))
		}
		...
		if err != nil {
			ferrors.HandleAndExit(log, cfg.App.SuppressExitCodes, fmt.Errorf("cancelling process instance(s): %w", err))
		}
	},
}
