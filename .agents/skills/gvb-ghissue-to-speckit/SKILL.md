...
14. After the spec is generated, ensure the resulting `spec.md` includes explicit GitHub issue traceability:
   ...
   - issue URL
   - issue title
15. For any commit created as part of this workflow or by downstream skills acting on the generated feature, preserve Conventional Commits formatting and append `#<issue-number>` at the end of the commit subject line.
...
14. When reporting completion or handing off to downstream work, restate the commit rule explicitly: commit subjects must keep the Conventional Commit prefix and append `#<issue-number>` as the final token.
